  ...
  createUserWithEmailAndPassword(auth, email, password)
    .then((userCredential) => {
      ...
      .then(() => {
        ...
        // Creating a QuizTable each register
        ...
        setDoc(newQuizTableRef, {})
          .then(() => {
            ...
            // Creating sub-collections "quizzes" and "settings" for the new quiz table
            const quizzesRef = collection(db, `users/${user.uid}/quizTables/${newQuizTableRef.id}/quizzes`);
            const settingsRef = collection(db, `users/${user.uid}/quizTables/${newQuizTableRef.id}/settings`);
 
            setDoc(doc(quizzesRef), {})
              .then(() => {
                console.log("Quizzes sub-collection created");
              })
              .catch((error) => {
                console.error("Error creating quizzes sub-collection: ", error);
              });
 
            setDoc(doc(settingsRef), {})
              .then(() => {
                console.log("Settings sub-collection created");
              })
              .catch((error) => {
                console.error("Error creating settings sub-collection: ", error);
              });
          })
          .catch((error) => {
